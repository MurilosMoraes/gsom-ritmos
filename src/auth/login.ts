...
class LoginPage {
  ...
  private async init(): Promise<void> {
    ...
    if (looksLikeAuthLink) {
      ...
      // SÓ confirmação de cadastro segue o caminho novo. Qualquer outro link
      // (recovery, magic link, e principalmente link SEM type, como o
      // ?code= do PKCE) continua abrindo o form de nova senha, como sempre.
      // Tratar "sem type" como confirmação deixava o cliente que pediu
      // reset entrar na conta sem conseguir trocar a senha.
      const isSignupConfirm = authType === 'signup' || authType === 'email';
      ...
      this.showAlert(isSignupConfirm ? t('auth.login.confirmInvalid') : t('auth.login.recoveryInvalid'), 'error');
