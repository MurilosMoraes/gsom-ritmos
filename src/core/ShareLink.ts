...
export function showShareResultModal(url: string, title: string, typeLabel: string, testUrl?: string): void {
  ...
  overlay.innerHTML = `
    <div style="background:rgba(10,10,26,0.97);border:1px solid rgba(255,255,255,0.08);border-radius:20px;padding:1.5rem;max-width:420px;width:100%;">
      ...
      ${testUrl ? `<a href="${esc(testUrl)}" style="display:block;width:100%;box-sizing:border-box;text-align:center;padding:0.65rem;margin:0 0 0.7rem;border-radius:12px;background:rgba(250,204,21,0.14);border:1px solid rgba(250,204,21,0.45);color:#facc15;font-size:0.85rem;font-weight:800;text-decoration:none;">▶ Testar aqui (abre e importa neste aparelho)</a>
      <p style="font-size:0.68rem;color:rgba(255,255,255,0.35);text-align:center;margin:0 0 0.9rem;">O link acima só abre de verdade quando estiver no ar em produção</p>` : ''}
