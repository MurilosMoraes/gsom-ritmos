...
- **Suporte:** em pt-BR o link vai pro GRUPO da comunidade; em qualquer outro idioma vai pra conversa DIRETA no WhatsApp do atendimento (`wa.me/5547984639792`) com a primeira mensagem já escrita no idioma do cliente (`support.firstMessage`). Nada de jogar estrangeiro em grupo brasileiro, e nada de só `mailto:` (celular sem app de e-mail configurado não abre nada). Tudo em `supportHref()` no `src/i18n/index.ts`, coberto por `test/suporte-idioma-test.mts`; no HTML basta marcar o link com `data-support-link`. O e-mail contato@gdrums.com.br segue publicado como alternativa (landings, privacidade, termos e ficha das lojas).
