...
serve(async (req) => {
  ...
  const results = {
    trial_24h: { candidates: 0, onesignal: 0, fcm: 0, failed: 0 },
    expired_today: { candidates: 0, onesignal: 0, fcm: 0, failed: 0 },
  };
  ...
  const runBatch = async (
    ...
    bucket: { candidates: number; onesignal: number; fcm: number; failed: number },
  ) => {
  ...
  try {
    ...
    await runBatch(
      ...
      "Seu teste acaba amanhã",
      "Garante os 166 ritmos e o pedal antes do ensaio. Ativa o plano agora.",
  ...
  try {
    ...
    await runBatch(
      ...
      "Seu teste acabou",
      "Volta pros 166 ritmos. No plano anual sai por R$ 19 por mês, 34% menos que o mensal.",
