...
## 🆕 17/set/2026: Fase 2 trazida pra cima da main atual

A `feature/intl-signup` foi reaplicada sobre a main de setembro (191 commits à frente) e revisada. Mudanças em relação ao que está descrito abaixo:

- **Função `register-account` v12:** caminho BR copiado da produção (v11, com as guardas de e-mail "gmail.come" e descartável que a branch não tinha). O internacional também passa por essas guardas. Provado por `node test/register-account-test.mjs` (47 casos): no BR, resposta e banco **idênticos** à v11 em 17 cenários.
- **Bug corrigido no rate limit:** contava também as linhas `created`/`blocked`, então o limite de 5/hora virava 3/hora. Agora conta só `attempt`.
- **Regressão evitada no login:** link de recuperação **sem** `type` (ex.: `?code=` do PKCE) caía no caminho "cadastro confirmado" e o cliente entrava sem trocar a senha. Agora só `type=signup|email` segue o caminho novo.
- **Conta incompleta:** migration `20260917_complete_profile_country_phone_optional.sql`. `check_email_status` só marca incompleta conta **BR sem CPF** (antes prendia todo estrangeiro e todo BR sem telefone); `complete_my_profile` aceita telefone vazio. `main.ts` também deixou de exigir telefone. Testado em transação desfeita em produção (19 casos).
- **Cadastro:** país padrão pelo **fuso horário** (brasileiro com celular em inglês não vira "EUA"); telefone e nome em formato internacional; limite de caracteres do telefone por país; 3 textos da tela "confira seu e-mail" que não existiam no dicionário.

**Ordem de deploy:** migration → função v12 → site. O site novo manda telefone vazio pro `complete_my_profile`, que só a função nova aceita.
